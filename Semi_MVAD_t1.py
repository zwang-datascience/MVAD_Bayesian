# -*- coding: utf-8 -*-
"""
Created on Wed May 22 17:04:41 2019
...
for ratio_idx in range(len(ratios)):
    for dim_idx in range(len(dims)):
        ...
        for itr in range(itrs):
            ...
            # type-1 outliers (sample is far away from the normal cluster under multiple views)
            X = np.copy(sample)
            n_X = X.shape[0]
            n_outliers = int(n_X*outlier_ratio)
            idx = np.random.permutation(n_X)[:n_outliers]
            X[idx] = np.random.uniform(size = (n_outliers,n_feats))
            label_ProbM = -np.ones(n_X)
            label_ProbM[idx] = 1
            ...
            # abnormal data
            mask_ab = np.zeros(n_X,dtype=bool)
            mask_ab[idx] = True
            x_abnor = X[mask_ab]
...
#            print('itr:',itr)
        ...
        print("Multiple view type-1 outlier detection result:",'ratio=',ratios[ratio_idx],'dim=',dims[dim_idx])
...
np.savetxt('AUCs(RatioDim)_t1.csv',np.concatenate((np.asmatrix(AUC_mean).T,np.asmatrix(AUC_std).T),axis=1), delimiter=",")