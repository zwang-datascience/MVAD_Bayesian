# -*- coding: utf-8 -*-
"""
Created on Wed May 22 18:37:12 2019
...
for ratio_idx in range(len(ratios)):
    for dim_idx in range(len(dims)):
        ...
        for itr in range(itrs):
            ...
            # type-3 outliers (sample is in a cluster under one view, but far away from cluster under another view)
            ...
            X_Pos[idx1,n_feats_SView:] = np.random.uniform(size = (n_outliers,n_feats-n_feats_SView))
            X_Neg[idx2,n_feats_SView:] = np.random.uniform(size = (n_outliers,n_feats-n_feats_SView))
...
#            print('itr:',itr)
        ...
        print("Multiple view type-3 outlier detection result:",'ratio=',ratios[ratio_idx],'dim=',dims[dim_idx])
...
np.savetxt('AUCs(RatioDim)_t3.csv',np.concatenate((np.asmatrix(AUC_mean).T,np.asmatrix(AUC_std).T),axis=1), delimiter=",")